    ...
    const educationItems: EducationItem[] = [
        {
            ...
            description: "Graduated with a Bachelor of Science in Computer Engineering from Holy Trinity College, General Santos City, Philippines (March 2019). Gained a strong foundation in computer systems, programming, and hardware integration, which supports my expertise in web development and innovative technology solutions."
        }
    ]
                    ...
                    <div className={`transition-all duration-1000 ${isVisible ? 'opacity-100 translate-x-0':'opacity-0 -translate-x-10'}`}>
                        ...
                        <div className="space-y-8">
                            {educationItems.map((education, educationIndex) => {
                                return <div key={educationIndex} className={`border-l-2 border-green-500 pl-6 relative group transition-all duration-1000 ${isVisible ? 'opacity-100 translate-x-0':'opacity-0 -translate-x-10'}`}>
                                    ...
                                    <div className="text-green-400 text-sm font-semibold mb-2 group-hover:text-green-300 transition-all duration-300">{education.year}</div>
                                    <h3 className="text-xl font-black text-white mb-2 group-hover:text-green-400 transition-all duration-300">{education.course}</h3>
                                    <p className="text-gray-400 mb-2 group-hover:text-gray-300 transition-all duration-300">{education.institution}</p>
                                    <p className="text-gray-300 text-sm leading-relaxed group-hover:text-white transition-all duration-300">{education.description}</p>
                                </div>;
                            })}
                        </div>
                    </div>
