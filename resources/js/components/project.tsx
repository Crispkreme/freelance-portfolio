...
const project = () => {
    ...
    return (
        <section id="project" className="py-24 bg-slate-950 overflow-hidden">
            ...
            <div className="container mx-auto px-6 relative z-10">
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-16">
                    {projectItems.map((project, projectIndex) => {
                        return (
                            <div className={`group bg-slate-900 rounded-xl shadow-md hover:shadow-lg transition-all duration-1000 delay-600 overflow-hidden border border-slate-700 hover:border-green-500 hover:scale-105  ${isVisible ? "opacity-100 translate-y-0":"opacity-0 translate-y-10"}`} style={{ transitionDelay: `${(projectIndex + 3) * 150} ms`}}>
                                ...
                                <div className='p-6'>
                                    <div className='flex items-center justify-between mb-3'>
                                        <span className='bg-slate-800 text-gray-400 px-2 py-1 rounded-full text-xs font-medium'>{project.role}</span>
                                    </div>
                                    <h4 className='text-lg font-bold text-white mb-2 group-hover:text-green-400 transition-all duration-300'>{project.title}</h4>
                                    <p className='text-gray-300 text-sm mb-4 leading-relaxed'>{project.description.substring(0, 100)} ...</p>
                                    <div className='flex flex-wrap gap-1 mb-4'>
                                        {project.technologies.map((technology, technologyIndex) => {
                                            return <span className='bg-green-600 text-white px-2 py-1 rounded-full text-xs font-medium' >{technology}</span>
                                        })}
                                    </div>
                                </div>
                            </div>
